// components/admin/training-category/EditTrainingCategoryModal.tsx
'use client';
...
interface EditTrainingCategoryModalProps {
    ...
    initialData: TrainingCategory | null;
    isSubmitting: boolean;
}

export function EditTrainingCategoryModal({
    isOpen,
    onClose,
    onSubmit,
    initialData,
    isSubmitting,
}: EditTrainingCategoryModalProps) {
    ...
    useEffect(() => {
        if (isOpen && initialData) {
            setFormData({
                ...
                color: initialData.color || '#1b7936',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0B192C]/40 backdrop-blur-xs animate-in fade-in duration-150">
            <div className="bg-white rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 sm:p-8 shadow-2xl border border-slate-200">
                <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-6">
                    <div>
                        <h2 className="text-lg font-bold text-[#0B192C]">Edit Training Category</h2>
                        <p className="text-xs text-slate-500">Update category details</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                        ...
                        {initialData && (
                            <p className="text-xs text-slate-400 mt-1">
                                Current slug: <span className="font-mono">{initialData.slug}</span>
                            </p>
                        )}
                    </div>
                    ...
                    <div className="flex gap-3 pt-4 border-t border-slate-100">
                        ...
                        >
                            ...
                            {isSubmitting || isProcessing ? 'Updating...' : 'Update Category'}
                        </button>
                        <button
                            type="button"
                            onClick={onClose}
